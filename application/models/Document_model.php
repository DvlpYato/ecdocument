...
class Document_model extends CI_Model
{
	...
	{
		$this->db->select('documents.*,categories.name,users.display_name,department.d_name');
		$this->db->from('documents');
		$this->db->join('categories','categories.id=documents.categorie_id');
		$this->db->join('users','users.id=documents.created_by');
		$this->db->join('department','department.department_id=documents.d_id');
	...
	public function entry_document($id,$filename = '')
	{
		$data = array(
			...
			'd_id' => $this->input->post('department_id'),
